...
class AssetPacker {
	public function new (){
		//Set console encoding to show colours and ASCII.
		Sys.command("@ECHO OFF >NUL");
		Sys.command("@chcp 65001>NUL");
		Sys.println(Colour.BOLD + Colour.GREEN+"
	 █████╗ ███████╗███████╗███████╗████████╗
	██╔══██╗██╔════╝██╔════╝██╔════╝╚══██╔══╝
	███████║███████╗███████╗█████╗     ██║
	██╔══██║╚════██║╚════██║██╔══╝     ██║
	...
	╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝   ╚═╝
		...
		if (log) Sys.println("Found "+files.length+" assets.");
		if (folderContents.nonPngs > 0){
			if (log) Sys.println("Ignoring "+folderContents.nonPngs+" non .png file(s).");
		}
		Sys.println("");
	...
	public function packImages (files:Array<String>,log=true,maxWidth:Int,maxHeight:Int){
		...
		var outputData:Array<{path:String,x:Int,y:Int,w:Int,h:Int}> = [];
		
		var index = 0;
		for( ff in files){
			index++;
			var path = ff;
			if (log) Sys.println(Colour.BOLD_CYAN +"Processing: " + path+Colour.RESET);
			...
			if (rect == null){
				if (log) Sys.println(Colour.RED+"Failed to pack!"+Colour.RESET);
				continue;
			}
			if (log) Sys.println("Placed "+rectWidth+"x"+rectHeight+" image at "+rect.x+", "+rect.y);
			...
			outputData.push({x: Math.floor(rect.x), y: Math.floor(rect.y), w: headerData.width, h: headerData.height, path:path});
			images.push({x: Math.floor(rect.x), y: Math.floor(rect.y), width: headerData.width, height: headerData.height, bytes: bytes});
			...
			if (log) Sys.println('['+bar+']  '+index+" / "+files.length+"\n");
		}
	...
	function get(w:Int,bytes:haxe.io.Bytes,x,y){
		var i = ((y*w)+(x))*4;
		return { r: bytes.get(i+2), g: bytes.get(i+1), b: bytes.get(i), a: bytes.get(i+3)};
	}
